...
import { CalendarIcon, Download, Copy, ImageIcon } from "lucide-react";
...
function generatePlainText(
  ...
  const parts = [
    ...
    "",
    divider,
    `  Generated by MyWallet · ${todayStr()}`,
  ];
...
function generatePNG(
  ...
  if (showPayments) {
    ctx.fillStyle = "#6b7280";
    ctx.fillText(`Generated ${todayStr()}`, totalW / 2, 76);
  }
      ...
      >
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-border/40 bg-muted/10">
          ...
          >
            ...
            {exportType === "clipboard" ? "Copy" : exportType === "image" ? "Download PNG" : "Export"} ({filteredShifts.length})
          </Button>
        </div>
      </DialogContent>
    </Dialog>
