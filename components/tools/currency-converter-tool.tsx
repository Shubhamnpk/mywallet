...
import { Loader2, RefreshCcw, Coins, History, Info, Check, ChevronDown, ArrowLeftRight } from "lucide-react"
...
export function CurrencyConverterTool() {
  ...
  const getExchangeRate = useCallback(async (from: string, to: string) => {
    if (from === to) {
      return { rate: 1, date: new Date().toISOString().slice(0, 10), source: "cache" as const }
    }
    ...
    if (typeof rate !== "number") throw new Error("Could not read conversion rate")
  ...
  const handleCurrencyConversion = useCallback(async (showToast = true) => {
    ...
    try {
      ...
      if (showToast) toast.success(source === "cache" ? "Used cached live rate" : "Updated with latest live rate")
    } catch (error) {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => handleCurrencyConversion(false), 100)
    return () => clearTimeout(timeout)
  }, [fromCurrency, toCurrency, handleCurrencyConversion])
  ...
  return (
    <Card className="bg-card/45 backdrop-blur-md border border-border/40 shadow-xl rounded-2xl overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-primary/5 pointer-events-none" />
      <CardHeader className="pb-2 px-4 pt-3 border-b border-border/10 relative z-10">
        <CardTitle className="text-xs font-black uppercase tracking-wider flex items-center gap-1.5">
          <ArrowLeftRight className="w-3.5 h-3.5 text-primary" /> Currency Converter
        </CardTitle>
      </CardHeader>
      <CardContent className="p-4 space-y-6 relative z-10">
              ...
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <Input
                    ...
                    className="border-0 p-0 text-2xl h-auto font-bold focus-visible:ring-0 bg-transparent placeholder:text-muted-foreground/30 shadow-none ring-0 focus-visible:ring-offset-0"
                    placeholder="1,000"
                  />
        ...
        <Button
          type="button"
          className="w-full h-12 rounded-2xl font-bold bg-primary hover:opacity-90 active:scale-[0.98] transition-all shadow-lg shadow-primary/20"
